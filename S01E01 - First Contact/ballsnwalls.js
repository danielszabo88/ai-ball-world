...
function sendMessage(from, to, text) {
  world.messages.push(new Message(from, to, text));
}

function detectNearbyAgents(agent, radius = 80) {
  const nearby = [];
  for (let i = 0; i < world.balls.length; i++) {
    const other = world.balls[i];
...
function endConversation(agent1, agent2) {
  ...
  agent1.conversationEndTime = 0;
  agent2.conversationEndTime = 0;
...
function drawMessages() {
  ...
  world.messages = world.messages.filter((m) => m.isAlive());
