...
function startConversation(agent1, agent2) {
  ...
  agent1.changeRelationship(agent2, 1)
  agent2.changeRelationship(agent1, 1)
  conversation.count++;
...
function drawRelationships(){
  ...
  if(!agent || !agent.relationships) return
  for (const [id, value] of agent.relationships){
    const other = physicsWorld.balls.find((b) => b.id === id)
    if(!other) continue
    ctx.beginPath()
    ...
    ctx.font = "12px Arial"
    ctx.textAlign = "center"
    ctx.fillText(value.toFixed(1), midX, midY)
  }
  ctx.lineWidth = 1
}
