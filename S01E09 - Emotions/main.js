...
function startConversation(agent1, agent2) {
  ...
  agent1.addMemory("conversation", agent2, 2);
  agent2.addMemory("conversation", agent1, 2);
  conversation.count++;
...
function drawStateText(agent) {
  ctx.fillStyle = "black";
  ctx.font = "12px Arial";
  ctx.textAlign = "center";
  ctx.fillText(agent.state, agent.pos.x, agent.pos.y - 35);
}

function drawActionScores(agent) {
  const x = agent.pos.x;
  let y = agent.pos.y + 30;
  const entries = Object.entries(agent.actionScores);
  let bestName = null;
