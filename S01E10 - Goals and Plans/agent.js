class Agent extends Ball {
  constructor(name, x, y, r, m) {
    ...
    this.tasks = {
      ...
      request_conversation: {
        ...
        hasFailed: () => {
          return (
            !this.currentAgentTarget || Date.now() - this.requestSentAt > 3000
          );
        },
      },
      ...
      wait_for_response: {
        ...
        hasFailed: () => {
          return Date.now() - this.requestSentAt > 3000;
        },
      },
  ...
  resolveConversationRequest() {
    ...
    if (accepted) {
      startConversation(this, other);
      other.pendingConversation = null;
      other.requestSentAt = 0;
    } else {
  ...
  update(world) {
    ...
    if (this.currentPlan.length === 0 || shouldSwitch) {
      this.abortPlan();
