...
physicsWorld.gridWidth = Math.ceil(canvas.width / physicsWorld.cellSize);
physicsWorld.gridHeight = Math.ceil(canvas.height / physicsWorld.cellSize);
physicsWorld.gridSize = physicsWorld.gridWidth * physicsWorld.gridHeight;
physicsWorld.grid = new Array(physicsWorld.gridSize);
...
class Ball {
  ...
  drawBall() {
    ...
    ctx.stroke();
    if (physicsWorld.selectedBall === this) {
      if (this.target !== null) {
...
canvas.addEventListener("mousedown", function (e) {
  ...
  for (let i = 0; i < physicsWorld.balls.length; i++) {
    const b = physicsWorld.balls[i];
    if (distanceSquared({ x: mouseX, y: mouseY }, b.pos) <= b.r * b.r) {
      selectedBall = b;
      break;
    }
  }
  if (selectedBall) {
    if (physicsWorld.selectedBall === selectedBall) {
      physicsWorld.selectedBall = null;
      return;
    }
    physicsWorld.selectedBall = selectedBall;
    selectedBall.target = null;
    selectedBall.vel.set(0, 0);
  } else {
    const active = physicsWorld.selectedBall;
    if (active) {
...
canvas.addEventListener("contextmenu", function (e) {
  e.preventDefault();
  if (physicsWorld.selectedBall) {
    physicsWorld.selectedBall.target = null;
    physicsWorld.selectedBall.vel.set(0, 0);
    physicsWorld.selectedBall.acc.set(0, 0);
  }
  physicsWorld.selectedBall = null;
});
...
function handleInput() {
  for (const b of physicsWorld.balls) {
    ...
    if (physicsWorld.selectedBall === b) {
      keyControl(b);
      continue;
    }
    b.acc.set(0, 0);
...
function updatePhysics() {
  physicsWorld.balls.forEach((b) => b.reposition());
}
...
function buildSpatialGrid() {
  // clear grid
  for (let i = 0; i < physicsWorld.gridSize; i++) {
    physicsWorld.grid[i] = [];
  }
  // insert balls
  for (let i = 0; i < physicsWorld.balls.length; i++) {
    const b = physicsWorld.balls[i];
    const minX = Math.floor((b.pos.x - b.r) / physicsWorld.cellSize);
    const maxX = Math.floor((b.pos.x + b.r) / physicsWorld.cellSize);
    const minY = Math.floor((b.pos.y - b.r) / physicsWorld.cellSize);
    const maxY = Math.floor((b.pos.y + b.r) / physicsWorld.cellSize);
    for (let x = minX; x <= maxX; x++) {
      for (let y = minY; y <= maxY; y++) {
        if (
          x >= 0 &&
          y >= 0 &&
          x < physicsWorld.gridWidth &&
          y < physicsWorld.gridHeight
        ) {
          const index = x + y * physicsWorld.gridWidth;
          physicsWorld.grid[index].push(b);
        }
      }
    }
  }
...
function getNearbyBalls(pos, radius) {
  const result = [];
  const seen = new Set();
  const minX = Math.floor((pos.x - radius) / physicsWorld.cellSize);
  const maxX = Math.floor((pos.x + radius) / physicsWorld.cellSize);
  const minY = Math.floor((pos.y - radius) / physicsWorld.cellSize);
  const maxY = Math.floor((pos.y + radius) / physicsWorld.cellSize);
  for (let x = minX; x <= maxX; x++) {
    for (let y = minY; y <= maxY; y++) {
      if (
        x < 0 ||
        ...
        continue;
      const cell = physicsWorld.grid[x + y * physicsWorld.gridWidth];
      if (!cell) continue;
      for (const b of cell) {
        if (!seen.has(b.id)) {
          seen.add(b.id);
...
function resolveBallBallPenetration(b1, b2) {
  ...
  const distSq = distanceSquared(b1.pos, b2.pos);
  let dist;
  if (distSq === 0) return;
...
function solveCollisions() {
  ...
  for (let i = 0; i < physicsWorld.gridSize; i++) {
    ...
    // neighbors
    const cx = i % physicsWorld.gridWidth;
    const cy = (i / physicsWorld.gridWidth) | 0;
    for (let dx = 0; dx <= 1; dx++) {
      for (let dy = -1; dy <= 1; dy++) {
        ...
        if (
          nx < 0 ||
          ny < 0 ||
          nx >= physicsWorld.gridWidth ||
          ny >= physicsWorld.gridHeight
        )
          continue;
        const ni = nx + ny * physicsWorld.gridWidth;
        const ncell = grid[ni];
        if (!ncell) continue;
        for (let a = 0; a < len; a++) {
          const b1 = cell[a];
  ...
  for (let i = 0; i < physicsWorld.balls.length; i++) {
    const b = physicsWorld.balls[i];
    for (let j = 0; j < physicsWorld.walls.length; j++) {
      const w = physicsWorld.walls[j];
...
// ====================================================================
// RENDERING — physics-layer drawing
// ====================================================================
...
const UI_RENDER_INTERVAL = 6;

function render() {
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  if (window.renderBackground) {
  ...
  physicsWorld.walls.forEach((w) => w.drawWall());
  drawTargets();
  if (window.renderOverlay) {
    window.renderOverlay();
  }
  uiFrameCounter++;
  if (window.renderUI && uiFrameCounter % UI_RENDER_INTERVAL === 0) {
    window.renderUI();
  }
}
...
function mainLoop(time) {
  ...
  if (time - fpsLastSample >= 500) {
    currentFps = Math.round((fpsFrameCount * 1000) / (time - fpsLastSample));
    fpsFrameCount = 0;
    fpsLastSample = time;
  }
  if (!simControls.paused) {
    accumulator += frameTime * simControls.speedMultiplier;
    accumulator = Math.min(accumulator, 0.25 * simControls.speedMultiplier);
    handleInput();
    let steps = 0;
    const MAX_STEPS = 5;
