...
const GOAL_DESCRIPTIONS = {
  eat: "Hunting for food",
  rest: "Heading home to rest",
  visit_pub: (pub) => `Heading to ${pub.label}`,
  socialize: "Looking for someone to talk to",
...
class Agent extends Ball {
  constructor(name, x, y, r, m) {
    ...
    this.lastFoodSeen = null;
    this.currentPubTarget = null;
    this.pendingTarget = null;
    this.routingTargetSet = false;
    ...
    this.favoritePub = this.pickFavoritePub();

    // === Planner state ===
    this.goalCooldownUntil = 0;
    this.goalScores = {};
    this.currentGoal = null;
    this.interruptedGoal = null;
    this.currentPlan = [];
    this.currentTaskIndex = 0;
    this.plans = {
      wander: ["exit_space", "wander"],
      rest: ["enter_home", "rest"],
      visit_pub: ["choose_pub", "enter_pub"],
      eat: ["exit_space", "find_food", "move_to_food", "eat_food"],
      socialize: [
        "find_partner",
        "approach_partner",
    ...
    this.tasks = {
      ...
      exit_space: {
        execute: () => {
          ...
          const space = spaceContaining(this.pos.x, this.pos.y);
          if (!space) return;
          const door = space.getEntryPoint();
          this.setTarget(door.x, door.y);
        },
        isComplete: () => spaceContaining(this.pos.x, this.pos.y) == null,
        hasFailed: () => false,
      },
      ...
      enter_home: {
        execute: () => {
          ...
          if (this.routingTargetSet) return;
          const inside = home.getInteriorPoint();
          this.routeTo(inside.x, inside.y);
        },
        isComplete: () => {
          const home = this.home();
          return home != null && home.contains(this.pos.x, this.pos.y);
        },
        hasFailed: () => {
          return !this.home();
        },
      ...
      choose_pub: {
        execute: () => {
          if (this.currentPubTarget) return;
          this.favoritePub = this.pickFavoritePub();
          this.currentPubTarget = this.favoritePub;
        },
        isComplete: () => this.currentPubTarget != null,
        hasFailed: () => !this.favoritePub,
      },
      ...
      enter_pub: {
        execute: () => {
          if (!this.currentPubTarget) return;
          if (this.routingTargetSet) return;
          const inside = this.currentPubTarget.getInteriorPoint();
          this.routeTo(inside.x, inside.y);
        },
        isComplete: () => {
          return (
            this.currentPubTarget != null &&
            this.currentPubTarget.contains(this.pos.x, this.pos.y)
          );
        },
        hasFailed: () => !this.currentPubTarget,
      },
      ...
      converse: {
        execute: () => {
          ...
          const content = this.chooseMessageContent();
          sendMessage(
            this,
            this.conversationPartner,
            content.type,
  ...
  pickSearchTarget(radius = 150) {
    const margin = 50;
    const currentSpace = spaceContaining(this.pos.x, this.pos.y);
    let x, y;
    if (currentSpace) {
      const door = currentSpace.getEntryPoint();
      x = door.x + randInt(-radius, radius);
      y = door.y + randInt(-radius, radius);
    } else {
      x = this.pos.x + randInt(-radius, radius);
      y = this.pos.y + randInt(-radius, radius);
    }
    const clampedX = Math.max(margin, Math.min(canvas.width - margin, x));
    const clampedY = Math.max(margin, Math.min(canvas.height - 150, y));
    this.routingTargetSet = false;
    this.routeTo(clampedX, clampedY);
  }
  search() {
    if (!this.target) {
      this.pickSearchTarget();
  ...
  routeTo(x, y) {
    if (this.routingTargetSet) return;
    const currentSpace = spaceContaining(this.pos.x, this.pos.y);
    const destSpace = spaceContaining(x, y);
    if (currentSpace !== destSpace) {
      const doorSpace = currentSpace || destSpace;
      const door = doorSpace.getEntryPoint();
      this.setTarget(door.x, door.y);
      this.pendingTarget = { x, y };
    } else {
      this.setTarget(x, y);
      this.pendingTarget = null;
    }
    this.routingTargetSet = true;
  }
  resolvePendingTarget() {
    if (this.target == null && this.pendingTarget) {
      const dest = this.pendingTarget;
      this.pendingTarget = null;
      this.setTarget(dest.x, dest.y);
    }
  }
  pickFavoritePub() {
    let closest = null;
    let closestDist = Infinity;
    for (const pub of agentWorld.pubs) {
      const c = pub.center();
      const d = distanceSquared(this.pos, c);
      if (d < closestDist) {
        closestDist = d;
        closest = pub;
      }
  ...
  // MEMORY
  // ====================================================================
  addMemory(type, agent, value, importance = 1, source = "direct") {
    if (type === "collision") {
    ...
    const mem = new Memory(type, agent, value, importance, source);
  ...
  onMessageReceived(msg) {
    switch (msg.type) {
      case MESSAGE_TYPES.SHARE_LOCATION:
        this.addMemory("heard_location", msg.from, 1, 1, "indirect");
        break;
      case MESSAGE_TYPES.SHARE_GOAL:
        this.addMemory("heard_goal", msg.from, 1, 1, "indirect");
        break;
      case MESSAGE_TYPES.SHARE_NEED:
        this.addMemory("heard_need", msg.from, 1, 1, "indirect");
        break;
    }
  }
  ...
  scoreVisitPub() {
    ...
    let score = missingSocial;
    score *= 1 + this.emotions.frustration * 0.01;
    score *= 1 + this.emotions.stress * 0.01;
    return score;
  }
  scoreSocialize() {
  ...
  evaluateGoals() {
    return {
      ...
      visit_pub: clamp(this.scoreVisitPub(), 0, 100),
  ...
  forceGoal(goalName, holdMs = 10000) {
    if (this.isConversing && this.conversationPartner) {
      endConversation(
        this,
        this.conversationPartner,
        `${this.name} forced to ${goalName}`,
      );
    }
    this.abortPlan();
    this.startPlan(goalName);
    this.goalCooldownUntil = Date.now() + holdMs;
  }
  ...
  resolveConversationRequest() {
    ...
    if (accepted && startConversation(this, other)) {
      ...
      sendMessage(
        other,
        this,
        MESSAGE_TYPES.SMALL_TALK,
        null,
        MESSAGE_TEXT.REJECTION,
      );
  ...
  chooseMessageContent() {
    ...
    if (this.interruptedGoal) {
      const description =
        this.interruptedGoal === "visit_pub" && this.currentPubTarget
          ? GOAL_DESCRIPTIONS.visit_pub(this.currentPubTarget)
          : GOAL_DESCRIPTIONS[this.interruptedGoal];
      return {
        type: MESSAGE_TYPES.SHARE_GOAL,
        subject: this.interruptedGoal,
        text: description,
      };
    }
    return {
  ...
  display() {
    ...
    ctx.fillStyle = "black";
    ctx.fillText(
      this.favoritePub ? this.favoritePub.label.split(" ")[1] : "-",
      this.pos.x,
      this.pos.y,
    );
  }
  ...
  update() {
    ...
    const shouldSwitch =
      bestGoal !== this.currentGoal && now >= this.goalCooldownUntil;
    if (
      !this.isConversing &&
      !this.pendingIncomingRequest &&
      !this.pendingConversation &&
      ...
      this.goalCooldownUntil = now + 2000;
    }
    this.updatePlan();
  }
}
