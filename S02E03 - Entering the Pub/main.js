...
const HOME_LAYOUT = [
  { x: 70, y: 550, width: 150, height: 90 },
  { x: 405, y: 550, width: 150, height: 90 },
  { x: 740, y: 550, width: 150, height: 90 },
];

const PUB_LAYOUT = [
  { x: 240, y: 0, width: 120, height: 100 },
  { x: 420, y: 0, width: 120, height: 100 },
  { x: 600, y: 0, width: 120, height: 100 },
];
...
// ====================================================================
class Memory {
  constructor(type, agent, value, importance = 1, source = "direct") {
    this.type = type;
...
class Message {
  constructor(from, to, type, subject, text) {
    this.from = from;
    this.to = to;
    this.type = type;
...
class Space {
  ...
  doorSide() {
    ...
    if (this.width >= this.height) {
      return canvasCenter.y > c.y ? "bottom" : "top";
    } else {
      return canvasCenter.x > c.x ? "right" : "left";
    }
  }
  buildWalls() {
    const doorSide = this.doorSide();
    const x1 = this.x;
    const y1 = this.y;
    ...
    delete sides[doorSide];
    this.walls = Object.values(sides).map(
      ([a, b, c, d]) => new Wall(a, b, c, d),
    );
  }
  doorPoint(offset) {
...
// WORLD HELPERS
// ====================================================================
function sendMessage(from, to, type, subject, text) {
  const msg = new Message(from, to, type, subject, text);
  agentWorld.messages.push(msg);
  to.onMessageReceived(msg);
}
function getAgents() {
...
function getAgentsIn(space) {
  return getAgents().filter((a) => space.contains(a.pos.x, a.pos.y));
}
function spaceContaining(x, y) {
  const all = [...agentWorld.homes, ...agentWorld.pubs];
  return all.find((space) => space.contains(x, y));
}
...
function startConversation(agent1, agent2) {
  ...
  for (const [agent, partner] of [
    ...
    agent.interruptedGoal = agent.currentGoal;
...
function drawMessages() {
  ...
  agentWorld.messages.forEach((m) => {
    ...
    ctx.fillStyle = m.type === MESSAGE_TYPES.SMALL_TALK ? "#ffe6ec" : "#e3f2ff";
...
HOME_LAYOUT.forEach((h, i) => {
  const home = new Home(h.x, h.y, h.width, h.height);
  home.buildWalls();
  agentWorld.homes.push(home);
});
